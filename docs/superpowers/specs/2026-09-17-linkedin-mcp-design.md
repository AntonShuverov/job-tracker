...
- Сервер не зависит от модели. Вся логика (поиск, база, лимиты, публикация, экспорт) — в обычных
  модулях с функциями, которые можно вызвать без MCP; `server.py` — тонкая обёртка над ними.
  Это позволяет позже добавить скрипт-автозапуск с другой моделью (например, DeepSeek API,
  OpenAI-совместимый) поверх той же `linkedin.db` без переделки сервера. Сам автозапуск — вне рамок.
